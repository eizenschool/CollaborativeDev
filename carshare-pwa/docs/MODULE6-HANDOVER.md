...
| `placePhotos.js` | Shared image-size tiers. `shared/PlacePhotoService.js` gets current photo metadata from Maps JavaScript Places; stored photo names are never used to build image URLs. |
...
| **Opt-in media — read this before presenting** | Photos and Street View are **off by default** (`mediaMode.js`) — every card shows the illustration until the "Photos off" chip in the Explore control row is clicked. You can also select "View real photo" on an individual home card, or "Load Street View" on that detail frame. The setting persists in `localStorage`. |
| Real photos (live mode) | With media off, a card's `View real photo` button loads only that destination. With media on, visible cards resolve a current Google photo; failed images stay on the illustration and can be retried. |
