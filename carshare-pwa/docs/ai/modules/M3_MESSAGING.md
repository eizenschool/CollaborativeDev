...
- Database: `database/sql/016_m3_supabase_messaging.sql`; `017_m3_advisor_followup.sql` covers the direct-user foreign key; `018_m3_versioned_media_paths.sql` finalizes sender/conversation/message/version object paths; `025_m3_add_voice_messages.sql` adds private standalone audio attachments and their duration contract; `026_m3_add_wav_voice_fallback.sql` adds the playback-safe WAV fallback; `033_project_notifications.sql` extends `send_message` so every other active conversation member receives one shared-centre notification; undeployed `035_m3_message_translation.sql` adds the member-readable, Edge-Function-written shared translation cache.
...
Clients receive SELECT-only grants on messaging and translation-cache tables. All message mutations use narrow authenticated `SECURITY DEFINER` RPCs with empty search paths, `auth.uid()` ownership/membership checks, lifecycle checks, and row locks. Translation-cache writes use the Edge Runtime's server credential only after a separately authenticated user, active membership, message tombstone, and conversation-expiry check. The `message-media` bucket is private: uploads are owner/conversation staged, listing is blocked, and downloads require a current visible attachment row or the same verified Edge Function boundary.

## Deferred

Phone/WebRTC calling, telephone-number access, email notifications, hazard advisories, address geocoding, and map point selection. Translation/UC3.6 code is complete but remains undeployed until `035`, the Edge Function, and the project-owner Cloudflare/Supabase secrets are applied. The shared in-app/Web Push notification integration is deployed in `033`; it still requires the project-owner VAPID secrets, Database Webhook, and two-account physical-device validation. Two-account cross-browser manual acceptance plus physical-device camera/microphone/translation checks remain required before a release sign-off.
