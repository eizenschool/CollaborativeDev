...
Resolved:
- Authoritative trip source is Module 2. History merges hosted rides with rides
  the user has an **Accepted** `ride_request` on; the module's own
...
Module 5 has no Supabase branch and no tables. It is blocked on a real
`Completed` transition: `transition_verified_ride()` is `service_role`-only with
no caller in `src/`, so against live Supabase every eco surface would be empty.
