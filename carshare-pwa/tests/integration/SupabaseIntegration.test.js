...
import { buildSignUpResult } from '../../src/business-logic/m1-profile/AuthService.js';
import { mapProfileRow } from '../../src/business-logic/m1-profile/ProfileService.js';
import { buildVehicleRecord } from '../../src/business-logic/m1-profile/VehicleService.js';
import {
  buildRideInsert,
  buildRidePatch,
  buildRideRpcArgs,
  mapRideRow,
  validateRideDraft
} from '../../src/business-logic/m2-rides/RideService.js';
import { DemoClockService } from '../../src/business-logic/m2-rides/verification/DemoClockService.js';
...
describe('Supabase integration contracts', () => {
  ...
  it('falls back to sender initials when an external message avatar fails', async () => {
    const messageBubbleSource = await import('node:fs/promises').then(({ readFile }) => readFile(
      new URL('../../src/presentation/m3-messaging/components/MessageBubble.jsx', import.meta.url),
      'utf8'
    ));
