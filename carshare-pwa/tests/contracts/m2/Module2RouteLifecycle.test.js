...
const locationInputUrl = new URL('../../../src/presentation/shared/components/maps/ConfirmedLocationInput.jsx', import.meta.url);
const rideServiceUrl = new URL('../../../src/business-logic/m2-rides/RideService.js', import.meta.url);
const rideRequestServiceUrl = new URL('../../../src/business-logic/m2-rides/RideRequestService.js', import.meta.url);
const rideAdapterUrl = new URL('../../../src/data-access/m2-rides/rideSupabaseAdapter.js', import.meta.url);
const rideRequestAdapterUrl = new URL('../../../src/data-access/m2-rides/rideRequestSupabaseAdapter.js', import.meta.url);
...
describe('Module 2 route schedule and lifecycle contracts', () => {
  ...
  it('keeps the undeployed-027 Host edit fallback private and vehicle-aware', async () => {
    const [service, adapter] = await Promise.all([
      ...
      readFile(rideAdapterUrl, 'utf8')
    ]);
    expect(adapter).toContain('LEGACY_HOST_RIDE_SELECT');
  ...
  it('disambiguates every ride host profile embed by its foreign key', async () => {
    const [rideService, rideRequestService] = await Promise.all([
      readFile(rideAdapterUrl, 'utf8'),
      readFile(rideRequestAdapterUrl, 'utf8')
    ]);
