...
let normalizeProfileVisibility;

let configured = false;

export function configureLegacyMockDataStore(dependencies) {
...
export const mockDb = {
  ...
  async submitIdentityVerification(userId, submission) {
    ...
    const documentType = submission.documentType || 'mykad';
    const icNumber = documentType === 'passport' ? null : (submission?.icNumber || '').replace(/[\s-]/g, '');
    ...
    const heldByAnotherAccount = Object.entries(db.identityVerifications).some(
      ([otherUserId, record]) => icNumber && otherUserId !== userId && record.ic_number === icNumber
    );
    ...
    db.identityVerifications[userId] = {
      status: 'pending',
      document_path: submission.file ? `${userId}/${documentType}-${Date.now()}.mock` : db.identityVerifications[userId]?.document_path,
      document_type: documentType,
      passport_number: documentType === 'passport' ? submission.documentNumber : null,
