...
import {
  GUIDE_FIXTURE_STORAGE_KEY,
  GUIDE_SESSION_RETENTION_DAYS,
} from './guidePersistenceConfig.js';
...
function read() {
  ...
  try {
    const parsed = JSON.parse(target.getItem(GUIDE_FIXTURE_STORAGE_KEY) || 'null');
...
function write(db) {
  const target = storage();
  if (target) target.setItem(GUIDE_FIXTURE_STORAGE_KEY, JSON.stringify(db));
...
function prune(db, now = Date.now()) {
  const cutoff = now - GUIDE_SESSION_RETENTION_DAYS * 86400000;
