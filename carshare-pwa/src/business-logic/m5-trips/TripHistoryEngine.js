...
import { tripHistorySupabaseAdapter } from '../../data-access/m5-trips/tripHistorySupabaseAdapter.js';
import '../shared/fixture/legacyMockDb.js';
import { tripHistoryMockAdapter } from '../../data-access/m5-trips/tripHistoryMockAdapter.js';
import { RideService } from '../m2-rides/RideService.js';
import { RideRequestService } from '../m2-rides/RideRequestService.js';
import { HostImpactEngine } from '../m1-profile/HostImpactEngine.js';
import { departureParts } from '../m2-rides/rideDateTime.js';
...
// UC5.3's participant list. The injected M2 request adapter deliberately refuses
...
export const TripHistoryEngine = {
  backend: tripHistorySupabaseAdapter.isConfigured ? 'supabase' : 'mock',
  ...
  async getLeaderboard(userId, year, month, now = new Date()) {
    return tripHistorySupabaseAdapter.isConfigured
...
async function liveLeaderboard(userId) {
  const { data, error } = await tripHistorySupabaseAdapter.listHostsWithCompletedTrips();
  ...
  const { data: profiles } = await tripHistorySupabaseAdapter.listProfileNames(hostIds);
...
async function mockLeaderboard(userId, year, month, now) {
  ...
  const [allRides, currentUser] = await Promise.all([tripHistoryMockAdapter.listAllRides(), tripHistoryMockAdapter.getCurrentUser()]);
