...
export const HostImpactEngine = {
  ...
  async getImpactSummary(userId) {
    const stats = reputationSupabaseAdapter.isConfigured
      ? await fetchFromSupabase(userId)
      : await profileMockAdapter.getImpactStats(userId);
  ...
  async applyDemoAdjustment(userId, { trips, reputation }) {
    ...
    await profileMockAdapter.adjustImpactStats(userId, { trips, reputation });
    return HostImpactEngine.getImpactSummary(userId);
  }
};

async function fetchFromSupabase(userId) {
  const data = await reputationSupabaseAdapter.getHostImpact(userId);
  return {
