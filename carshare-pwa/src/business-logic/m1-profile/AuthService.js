...
import { authSupabaseAdapter } from '../../data-access/m1-profile/authSupabaseAdapter.js';
...
export const AuthService = {
  backend: authSupabaseAdapter.isConfigured ? 'supabase' : 'mock',
  ...
  async signUp({ fullName, email, password }) {
    ...
    const user = await profileMockAdapter.signUp({ fullName, email, password });
  ...
  async signIn({ email, password }) {
    ...
    if (authSupabaseAdapter.isConfigured) {
      const { data, error } = await authSupabaseAdapter.signInWithPassword({ email, password });
      if (error) throw error;
      await ProfileService.reactivateAccount(data.user.id);
      return ProfileService.getProfile(data.user.id, data.user);
    }

    return profileMockAdapter.signIn({ email });
  },
  ...
  async reactivateOnSignIn(userId) {
    if (!authSupabaseAdapter.isConfigured) return;
  ...
  async signOut() {
    if (authSupabaseAdapter.isConfigured) {
      const { error } = await authSupabaseAdapter.signOut();
      if (error) throw error;
    }
  },
  ...
  async getCurrentUser() {
    if (authSupabaseAdapter.isConfigured) {
      const { data: sessionData, error: sessionError } = await authSupabaseAdapter.getSession();
      if (sessionError) throw sessionError;
      if (!sessionData.session) return null;
      const { data, error } = await authSupabaseAdapter.getUser();
      if (error) throw error;
  ...
  onAuthStateChange(listener) {
    if (!authSupabaseAdapter.isConfigured) return () => {};
    const { data } = authSupabaseAdapter.onAuthStateChange((event, session) => {
