...
import { rideLiveTrackingSupabaseAdapter } from '../../data-access/m2-rides/rideLiveTrackingSupabaseAdapter.js';
...
function assertConfigured() {
  if (!rideLiveTrackingSupabaseAdapter.isConfigured) throw new Error('Live tracking requires a configured Supabase connection.');
}
...
export const RideLiveTrackingService = {
  backend: rideLiveTrackingSupabaseAdapter.isConfigured ? 'supabase' : 'unconfigured',

  async startSharing(rideId, consentVersion = 'm2-live-v1') {
    return run(() => rideLiveTrackingSupabaseAdapter.startSharing(rideId, consentVersion), 'Unable to start location sharing.');
  },

  async publishLocation(rideId, point) {
    const result = await run(() => rideLiveTrackingSupabaseAdapter.publishLocation(rideId, point), 'Unable to publish your current location.');
    return result;
  },

  async stopSharing(rideId) {
    return run(() => rideLiveTrackingSupabaseAdapter.stopSharing(rideId), 'Unable to stop location sharing.');
  },

  async getLiveSnapshot(rideId) {
    const rows = await run(() => rideLiveTrackingSupabaseAdapter.getLiveSnapshot(rideId), 'Unable to load live locations.');
    return (rows || []).map(normalizePoint).filter(Boolean);
  },

  async getHistory(rideId, { after = null, limit = 2000 } = {}) {
    const rows = await run(() => rideLiveTrackingSupabaseAdapter.getHistory(rideId, after, limit), 'Unable to load trip location history.');
    return (rows || []).map(normalizePoint).filter(Boolean);
  },

  async hideMyHistory(rideId) {
    return run(() => rideLiveTrackingSupabaseAdapter.hideHistory(rideId), 'Unable to hide your route history.');
  },

  async createFamilyShare(rideId) {
    return run(() => rideLiveTrackingSupabaseAdapter.createFamilyShare(rideId), 'Unable to create a family location link.');
  },

  async revokeFamilyShare(shareId) {
    return run(() => rideLiveTrackingSupabaseAdapter.revokeFamilyShare(shareId), 'Unable to revoke the family location link.');
  },

  async consumeDynamicMapLoad(pageSessionId) {
    return run(() => rideLiveTrackingSupabaseAdapter.consumeDynamicMapLoad(pageSessionId), 'Unable to check map availability.');
  },

  subscribeLive(rideId, { isDriver = false, onPoint = () => {}, onStatus = () => {} } = {}) {
    assertConfigured();
    const suffix = isDriver ? 'host' : 'driver';
    return rideLiveTrackingSupabaseAdapter.subscribe(rideId, suffix, (payload) => {
