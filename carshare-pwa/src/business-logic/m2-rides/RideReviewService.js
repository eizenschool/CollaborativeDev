...
import { rideReviewSupabaseAdapter } from '../../data-access/m2-rides/rideReviewSupabaseAdapter.js';
...
export const RideReviewService = {
  backend: rideReviewSupabaseAdapter.isConfigured ? 'supabase' : 'mock',

  async submitReview(reviewerId, { rideId, revieweeId, rating, comment = '' }) {
    const review = validateRideReview({ rating, comment });
    if (rideReviewSupabaseAdapter.isConfigured) {
      const { data: reviewId, error } = await rideReviewSupabaseAdapter.submit({ rideId, revieweeId, ...review });
      if (error) throw normalizeError(error);
      const { data, error: readError } = await rideReviewSupabaseAdapter.getById(reviewId);
      if (readError) throw normalizeError(readError);
      return mapReview(data);
    }
    return rideMockAdapter.submitRideReview(reviewerId, { rideId, revieweeId, ...review });
  },

  async listProfileReviews(profileId) {
    if (rideReviewSupabaseAdapter.isConfigured) {
      const { data, error } = await rideReviewSupabaseAdapter.listByReviewee(profileId);
      if (error) throw normalizeError(error);
      return data.map(mapReview);
    }
    return rideMockAdapter.listProfileReviews(profileId);
  },

  async getEligibility(reviewerId, rideId) {
    if (rideReviewSupabaseAdapter.isConfigured) {
      const { data, error } = await rideReviewSupabaseAdapter.getEligibility(rideId);
      if (error) throw normalizeError(error);
    ...
    return rideMockAdapter.getRideReviewEligibility(reviewerId, rideId);
  }
};
