...
import { rideSupabaseAdapter } from '../../data-access/m2-rides/rideSupabaseAdapter.js';
...
} from '../shared/GooglePlacesService.js';
...
async function invokeRouteFunction(body) {
  const { data, error } = await rideSupabaseAdapter.invokeRoute(body);
...
export async function attachDestinationPhotoPlaceIds(rides = []) {
  ...
  const { data, error } = await rideSupabaseAdapter.getDestinationPhotoPlaceIds(rideIds);
...
export const RideService = {
  backend: rideSupabaseAdapter.isConfigured ? 'supabase' : 'mock',
  ...
  async searchRides({ from, to, date, proximity = null, confirmedLocations = null, compatibility = null } = {}) {
    if (rideSupabaseAdapter.isConfigured) {
      ...
      const { data: compatibleData, error: compatibilityError } = await rideSupabaseAdapter.searchCompatible({
          p_pickup: from || null,
      ...
      if (proximity) {
        const { data, error } = await rideSupabaseAdapter.searchNearDestination({
      ...
      const search = (legacy = false) => rideSupabaseAdapter.searchPublished({ from, to, range, legacy });
      let { data, error } = await search();
      if (error && isUndeployedModule2Upgrade(error)) ({ data, error } = await search(true));
      if (error) throw rpcError(error);
      return attachDestinationPhotoPlaceIds(data.map(mapRideRow));
    }
    return attachDestinationPhotoPlaceIds(await rideMockAdapter.listRides({
  ...
  async listMyRides(userId) {
    if (rideSupabaseAdapter.isConfigured) {
      let { data, error } = await rideSupabaseAdapter.listByHost(userId);
      if (error && isUndeployedModule2Upgrade(error)) ({ data, error } = await rideSupabaseAdapter.listByHost(userId, { legacy: true }));
      if (error) throw rpcError(error);
      return { hosting: await attachDestinationPhotoPlaceIds(data.map(mapRideRow)), joining: [] };
    }
    const result = await rideMockAdapter.listMyRides(userId);
    return {
  ...
  async getRide(rideId) {
    if (rideSupabaseAdapter.isConfigured) {
      const { data: sessionData } = await rideSupabaseAdapter.getSession();
      let { data, error } = await rideSupabaseAdapter.getRide(rideId);
      if (error && isUndeployedModule2Upgrade(error)) ({ data, error } = await rideSupabaseAdapter.getRide(rideId, { mode: 'legacy-public' }));
      if (error) throw rpcError(error);
      if (!data) return null;
      if (sessionData.session) {
        const { data: privateData, error: privateError } = await rideSupabaseAdapter.getParticipantDetail(rideId);
        ...
        if (privateError && data.host_id === sessionData.session.user.id) {
          const { data: legacyHostData, error: legacyHostError } = await rideSupabaseAdapter.getRide(rideId, { mode: 'legacy-host' });
      ...
      const { data: publicContext } = await rideSupabaseAdapter.getPublicPickupContext(rideId);
  ...
  async updateRide(rideId, patch) {
    ...
    if (rideSupabaseAdapter.isConfigured) {
      ...
      const { error } = await rideSupabaseAdapter.updateRide(rideId, buildRideRpcArgs(merged));
      if (error) throw rpcError(error);
  ...
  async publishRide(hostId, rideData, status = 'Published') {
    ...
    return rideMockAdapter.createRide(hostId, { ...rideData, waypoints: normalizeWaypoints(rideData.waypoints) }, status);
  },
  ...
  async deleteDraft(rideId) {
    if (rideSupabaseAdapter.isConfigured) {
      const { error } = await rideSupabaseAdapter.deleteDraft(rideId);
      if (error) throw rpcError(error);
      return true;
    }
    return rideMockAdapter.deleteDraft(rideId);
  },

  async republishAsDraft(rideId) {
    if (rideSupabaseAdapter.isConfigured) {
      const { data: newRideId, error } = await rideSupabaseAdapter.republishAsDraft(rideId);
      if (error) throw rpcError(error);
      return this.getRide(newRideId);
    }
    return rideMockAdapter.republishRideAsDraft(rideId);
  },

  async cancelRide(rideId, reason) {
    if (rideSupabaseAdapter.isConfigured) {
      const { error } = await rideSupabaseAdapter.cancel(rideId, reason);
      if (error) throw rpcError(error);
      return this.getRide(rideId);
    }
    return rideMockAdapter.cancelRide(rideId, reason);
  },

  async closeRecruitment(rideId) {
    if (rideSupabaseAdapter.isConfigured) {
      const { error } = await rideSupabaseAdapter.closeRecruitment(rideId);
      if (error) throw rpcError(error);
      return this.getRide(rideId);
    }
    return rideMockAdapter.closeRideRecruitment(rideId);
  },

  async reopenRecruitment(rideId) {
    if (rideSupabaseAdapter.isConfigured) {
      const { error } = await rideSupabaseAdapter.reopenRecruitment(rideId);
      if (error) throw rpcError(error);
      return this.getRide(rideId);
    }
    return rideMockAdapter.reopenRideRecruitment(rideId);
  },

  async getLifecycleContext(rideId) {
    if (rideSupabaseAdapter.isConfigured) {
      const { data, error } = await rideSupabaseAdapter.getLifecycleContext(rideId);
      if (error) throw rpcError(error);
    ...
    return rideMockAdapter.getRideLifecycleContext(rideId);
  },
  ...
  async startRide(rideId) {
    ...
    return rideMockAdapter.startRide(rideId);
  },

  async confirmDriverArrival(rideId) {
    const position = await accuratePosition();
    if (rideSupabaseAdapter.isConfigured) {
      const { error } = await rideSupabaseAdapter.confirmDriverArrival(rideId, position);
      if (error) throw rpcError(error);
      return this.getRide(rideId);
    }
    return rideMockAdapter.confirmDriverArrival(rideId, position);
  }
};
