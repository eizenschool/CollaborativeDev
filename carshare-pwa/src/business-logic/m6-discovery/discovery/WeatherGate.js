...
//   - fetchForecasts() turns adapter rows into forecast objects; HTTP transport
//     stays in Module 6 Data Access.
//
...
import { fetchWeatherForecastRows } from '../../../data-access/m6-discovery/weatherForecastAdapter.js';
 ...
 */
export async function fetchForecasts(places = [], travelDate, { fetchImpl } = {}) {
  ...
  try {
    const rows = await fetchWeatherForecastRows(missing, travelDate, { fetchImpl });
    rows.forEach(({ placeId, weatherCode }) => {
      if (weatherCode === undefined || weatherCode === null) return;

      const forecast = { weatherCode, summary: describeCode(weatherCode) };
      forecastCache.set(cacheKey(placeId, travelDate), forecast);
      found.set(placeId, forecast);
    });
