...
import { fetchStreetViewCoverage } from '../../../data-access/m6-discovery/streetViewCoverageAdapter.js';
...
export async function checkStreetViewCoverage(
  ...
  try {
    const body = await fetchStreetViewCoverage(lat, lng, { baseUrl, fetchImpl });
    if (!body) return NOT_COVERED;
