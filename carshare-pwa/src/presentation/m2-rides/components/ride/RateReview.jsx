...
import { IconArrowLeft, IconStar } from '../../../shared/components/icons.jsx';
  ...
  return (
    <main className="phone-ride-page review-page">
      ...
      <div className="review-content">
        ...
        {targets.length > 1 && <div className="chip-select-row" aria-label="Choose a person to review">{targets.map((item) => <button type="button" aria-pressed={selectedId === item.revieweeId} className={`chip-select ${selectedId === item.revieweeId ? 'active' : ''}`} key={item.revieweeId} onClick={() => { setSelectedId(item.revieweeId); setRating(0); setReview(''); }}>{item.revieweeName}</button>)}</div>}
        {!target ? <section className="empty-request-state"><strong>No review available</strong><p>Only the Host and accepted account holders can review one another after a Completed ride.</p></section> : <>
          ...
          {target.existingRating ? <section className="ride-info-card rating-card"><p className="eyebrow">YOUR REVIEW</p><div className="rating-stars">{[1, 2, 3, 4, 5].map((value) => <IconStar key={value} size={30} fill={value <= target.existingRating ? '#f59e0b' : 'none'} />)}</div><strong>{target.existingRating}/5</strong><p>{target.existingComment || 'No written comment'}</p><small>Submitted {new Date(target.reviewedAt).toLocaleDateString()}</small></section> : <>
            <section className="ride-info-card rating-card"><p className="eyebrow">HOW WAS YOUR EXPERIENCE?</p><div className="rating-stars" role="radiogroup" aria-label="Rating">{[1, 2, 3, 4, 5].map((value) => <button type="button" role="radio" aria-checked={rating === value} key={value} onClick={() => setRating(value)} onMouseEnter={() => setHovered(value)} onMouseLeave={() => setHovered(0)} aria-label={`${value} stars`}><IconStar size={36} fill={value <= displayRating ? '#f59e0b' : 'none'} /></button>)}</div>{displayRating > 0 && <strong aria-live="polite">{labels[displayRating]}</strong>}</section>
