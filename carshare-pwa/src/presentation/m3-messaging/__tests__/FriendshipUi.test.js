...
describe('friendship UI contract', () => {
  it('registers the static Friends route before the conversation-id route', () => {
    const app = read('src/presentation/shared/app/App.jsx');
  ...
  it('provides incoming, friend and sent sections without global member search', () => {
    const center = read('src/presentation/m3-messaging/components/FriendCenter.jsx');
  ...
  it('exposes all public-profile relationship actions and preserves auth return', () => {
    const profile = read('src/presentation/m1-profile/components/PublicProfile.jsx');
  ...
  it('labels friend chats separately and keeps removed chats visible as read-only', () => {
    const list = read('src/presentation/m3-messaging/components/ConversationList.jsx');
    const chat = read('src/presentation/m3-messaging/components/ChatWindow.jsx');
    const details = read('src/presentation/m3-messaging/components/ConversationDetailsContent.jsx');
