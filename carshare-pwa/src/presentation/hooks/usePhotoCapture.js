...
function cameraError(error) {
  ...
  if (error?.name === 'NotFoundError') return new Error('No camera was found on this device.');
  if (error?.name === 'NotReadableError') return new Error('The camera is already in use by another application.');
...
function photoFileFromVideo(video) {
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      if (!blob?.size) return reject(new Error('The camera returned an empty photo. Try again.'));
      resolve(new File([blob], `photo-${Date.now()}.jpg`, { type: 'image/jpeg' }));
    }, 'image/jpeg', 0.92);
  });
}

/** Shared in-page still-camera capture for Messaging and Ride photo fields. */
export default function usePhotoCapture({ onPhotoReady, onError }) {
  ...
  const startCapture = useCallback(async () => {
    ...
    if (!globalThis.isSecureContext) throw new Error('Taking a photo requires an HTTPS connection.');
    if (!navigator.mediaDevices?.getUserMedia) throw new Error('Taking a photo is not supported by this browser.');
