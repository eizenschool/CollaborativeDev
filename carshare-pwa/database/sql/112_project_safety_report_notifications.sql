...
-- Depends on 033_project_notifications.sql, 104_m1, 106_m1, 107_m1 and 109_m3.
...
begin
  ...
  select ride_id into v_ride
  from public.safety_reports
  where message_evidence_id = v_e.id
  limit 1;
  ...
  if p_remove and p_outcome <> 'dismissed' then
    ...
    if private.message_report_snapshot(v_e.message_id) = v_e.snapshot then
      ...
      set text_content = null, deleted_at = now(), moderated_at = now(), edited_at = null
  ...
  if p_outcome = 'warning' then
    ...
    perform private.create_user_notification(
      ...
      jsonb_build_object(
        ...
        'delta', v_delta,
        'rideId', v_ride,
        'messageRemoved', p_remove
      ),
      'message-report-subject:' || v_e.id::text
    );
  end if;

  update public.message_report_evidence
  set resolved_at = now(), decision = p_outcome
  where id = v_e.id;
