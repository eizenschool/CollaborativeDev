-- Module 3 voice compatibility: allow the 16 kHz mono PCM WAV fallback used
-- when Chromium/Electron produces WebM recordings without playable metadata.
...
alter table public.message_attachments
  add constraint message_attachments_mime_check check (
    ...
    or (kind = 'audio' and mime_type in ('audio/webm', 'audio/mp4', 'audio/ogg', 'audio/wav'))
  );
