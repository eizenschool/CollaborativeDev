-- Slightly wider passenger Check-in distance tolerance for ordinary browser
-- GPS drift. Accuracy must still be 150 m or better, submitted coordinates are
-- not stored, and Driver destination arrival keeps its stricter existing rule.
...
alter table public.ride_requests
  add constraint ride_requests_check_in_pair_check check (
    ...
    or (
      ...
      and check_in_distance_meters between 0 and 400
...
begin
  ...
  if v_distance::double precision > least(400::double precision, 250 + p_accuracy_meters) then
