-- Module 4: mandatory Google-confirmed pickup and destination search.
--
-- Passenger-selected Place IDs are inputs only. Privileged helpers compare
-- them against private Ride endpoint IDs and return the existing safe public
-- card/itinerary projections. Legacy rides without an endpoint ID may match
-- the confirmed display text; a different stored endpoint ID never falls back.

create index if not exists rides_public_pickup_place_idx
  on public.rides (pickup_place_id, departure_at)
  where status = 'Published' and seats_available > 0;

create index if not exists rides_public_destination_place_idx
  on public.rides (destination_place_id, departure_at)
  where status = 'Published' and seats_available > 0;

create function private.search_public_rides_with_confirmed_locations(
...
declare
  ...
  v_pickup_place_id text := nullif(btrim(p_pickup_place_id), '');
  v_destination_search_place_id text := nullif(btrim(p_destination_search_place_id), '');
  v_destination_place_id text := nullif(btrim(p_destination_place_id), '');
begin
  if v_pickup_place_id is null and v_destination_search_place_id is null then
    raise exception 'At least one confirmed location is required';
  end if;
  if v_pickup_place_id is not null and v_pickup is null then
    raise exception 'Confirmed pickup requires its display text';
  end if;
  if v_destination_search_place_id is not null and v_destination is null then
    raise exception 'Confirmed destination requires its display text';
  end if;
  if v_destination_search_place_id is not null and v_destination_place_id is not null then
    raise exception 'Exact destination and recommendation proximity cannot be combined';
  end if;
  if char_length(coalesce(v_pickup_place_id, '')) > 512
    or char_length(coalesce(v_destination_search_place_id, '')) > 512 then
  ...
  from private.search_public_rides_with_compatibility(
    case when v_pickup_place_id is null then v_pickup else null end,
    case when v_destination_search_place_id is null then v_destination else null end,
    p_departure_start,
    p_departure_end,
    v_destination_place_id,
    p_radius_km,
    p_vehicle_type,
    p_language
  ) c
  ...
  where (
      v_pickup_place_id is null
      or r.pickup_place_id = v_pickup_place_id
      or (
        r.pickup_place_id is null
        and r.pickup ilike '%' || v_pickup || '%'
      )
    )
    and (
      v_destination_search_place_id is null
      or r.destination_place_id = v_destination_search_place_id
      or (
        r.destination_place_id is null
        and r.destination ilike '%' || v_destination || '%'
      )
    )
  order by c.departure_at, c.ride_id;
end;
$$;

create function public.search_public_rides_with_confirmed_locations(
  p_pickup text default null,
  p_destination text default null,
  p_departure_start timestamptz default null,
  p_departure_end timestamptz default null,
...
as $$
  select *
  from private.search_public_rides_with_confirmed_locations(
...
revoke all on function private.search_public_rides_with_confirmed_locations(
  text, text, timestamptz, timestamptz, text, integer, text, text, text, text
) from public, anon, authenticated;
grant usage on schema private to anon, authenticated;
grant execute on function private.search_public_rides_with_confirmed_locations(
  text, text, timestamptz, timestamptz, text, integer, text, text, text, text
) to anon, authenticated;

revoke all on function public.search_public_rides_with_confirmed_locations(
  text, text, timestamptz, timestamptz, text, integer, text, text, text, text
) from public, anon, authenticated;
grant execute on function public.search_public_rides_with_confirmed_locations(
  text, text, timestamptz, timestamptz, text, integer, text, text, text, text
) to anon, authenticated;

create function private.search_public_multi_leg_journeys_with_confirmed_locations(
  p_pickup text,
  p_destination text default null,
  p_departure_start timestamptz default null,
  p_departure_end timestamptz default null,
...
begin
  ...
  if v_pickup_place_id is null or (v_destination_search_place_id is null and v_destination_place_id is null) then
    raise exception 'Multi-leg matching requires confirmed route endpoints';
  end if;
  if v_destination_search_place_id is not null and v_destination_place_id is not null then
    raise exception 'Exact destination and recommendation proximity cannot be combined';
  end if;
  if char_length(v_pickup) > 120 or char_length(coalesce(v_destination, '')) > 120
    or char_length(v_pickup_place_id) > 512
    or char_length(coalesce(v_destination_search_place_id, '')) > 512 then
    raise exception 'Search route value is too long';
  end if;
  ...
  matched as (
    ...
    where (
        l1.pickup_place_id = v_pickup_place_id
        or (l1.pickup_place_id is null and l1.pickup ilike '%' || v_pickup || '%')
      )
      ...
      and (
        (v_destination_place_id is not null and n.source_place_id is not null)
        or (
          v_destination_search_place_id is not null
          and (
            l2.destination_place_id = v_destination_search_place_id
            or (
              l2.destination_place_id is null
              and l2.destination ilike '%' || v_destination || '%'
            )
          )
        )
      )
...
create function public.search_public_multi_leg_journeys_with_confirmed_locations(
  p_pickup text,
  p_destination text default null,
  p_departure_start timestamptz default null,
  p_departure_end timestamptz default null,
...
as $$
  select *
  from private.search_public_multi_leg_journeys_with_confirmed_locations(
...
revoke all on function private.search_public_multi_leg_journeys_with_confirmed_locations(
  text, text, timestamptz, timestamptz, time, text, integer, text, integer,
  text[], text, numeric, text, text, text, text
) from public, anon, authenticated;
grant execute on function private.search_public_multi_leg_journeys_with_confirmed_locations(
  text, text, timestamptz, timestamptz, time, text, integer, text, integer,
  text[], text, numeric, text, text, text, text
) to anon, authenticated;

revoke all on function public.search_public_multi_leg_journeys_with_confirmed_locations(
  text, text, timestamptz, timestamptz, time, text, integer, text, integer,
  text[], text, numeric, text, text, text, text
) from public, anon, authenticated;
grant execute on function public.search_public_multi_leg_journeys_with_confirmed_locations(
  text, text, timestamptz, timestamptz, time, text, integer, text, integer,
  text[], text, numeric, text, text, text, text
) to anon, authenticated;
