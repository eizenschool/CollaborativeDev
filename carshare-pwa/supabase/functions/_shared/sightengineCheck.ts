...
export async function checkImageWithSightengine({
  ...
  fetchImpl = fetch, timeoutMs = 15_000, maxAttempts = 2, strict = false
}: {
  apiUser: string; apiSecret: string; imageBase64: string; mimeType?: string;
  fetchImpl?: typeof fetch; timeoutMs?: number; maxAttempts?: number; strict?: boolean;
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      if (strict && (body?.status !== 'success'
        || !['sexual_activity','sexual_display','erotica','very_suggestive'].every((key) => typeof body?.nudity?.[key] === 'number' && Number.isFinite(body.nudity[key]))
        || typeof body?.gore?.prob !== 'number'
        || !['nazi','asian_swastika','confederate','supremacist','terrorist','middle_finger'].every((key) => typeof body?.offensive?.[key] === 'number')
        || typeof body?.text?.content !== 'string')) throw new Error('Incomplete image verdict');
