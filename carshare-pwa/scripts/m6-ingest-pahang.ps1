# Pahang, one region only - same discipline as m6-ingest-johor.ps1
...
#   3. After -Step main, spot-check a few Pahang rows' category in the
#      Table Editor against what the place actually is.
#   4. Do not run -Step main a second time just to see it work.
#
# Usage:
#   $env:SUPABASE_SECRET_KEY = "sb_secret_..."
#   .\scripts\m6-ingest-pahang.ps1 -Step recon
#   .\scripts\m6-ingest-pahang.ps1 -Step main -MaxDetails 20
...
$region = @{ id = 'pahang'; state = 'Pahang'; latitude = 3.8168; longitude = 103.3260; radiusMeters = 50000 }
...
if ($Step -eq 'recon') {
  ...
  Write-Host "  .\scripts\m6-ingest-pahang.ps1 -Step main -MaxDetails <discovered, capped at 50>"
}
